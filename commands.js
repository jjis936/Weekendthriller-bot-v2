...
const slashCommands = [
    ...
    // -- Daily loot crate: chance at a random discount code --------------------
    {
        data: new SlashCommandBuilder()
            .setName("lootcrate")
            .setDescription("Open your daily crate for a chance at a discount code"),
        ...
        async execute(interaction){
            ...
            if(record.lastCrateOpened && now - record.lastCrateOpened < 24 * 60 * 60 * 1000){
                const hoursLeft = Math.ceil((24 * 60 * 60 * 1000 - (now - record.lastCrateOpened)) / (60 * 60 * 1000));
                return interaction.reply({ content: `⏳ You already opened a crate today. Come back in ~${hoursLeft}h.`, ephemeral: true });
            }

            const roll = Math.random() * 100;
            let rarity, percent, color, emoji;

            if(roll < 5){ rarity = "LEGENDARY"; percent = 35; color = "#FFD700"; emoji = "🌟"; }
            else if(roll < 20){ rarity = "EPIC"; percent = 20; color = "#A335EE"; emoji = "💜"; }
            else if(roll < 50){ rarity = "RARE"; percent = 10; color = "#0070DD"; emoji = "🔷"; }
            else{ rarity = "COMMON"; percent = 5; color = "#9D9D9D"; emoji = "⚪"; }

            const code = `CRATE-${interaction.user.username.slice(0, 4).toUpperCase()}${Math.floor(1000 + Math.random() * 9000)}`;

            const discounts = discountsDB.read();
            discounts[code] = { percent, usesLeft: 1, createdBy: "lootcrate" };
            discountsDB.write(discounts);

            record.lastCrateOpened = now;
            users[interaction.user.id] = record;
            usersDB.write(users);

            const embed = brandEmbed({
                title: `${emoji} ${rarity} CRATE OPENED!`,
                color,
                thumbnail: false,
                description:
                    `🎁 You pulled a **${rarity}** reward!\n\n` +
                    `## ${percent}% OFF\n\n` +
                    `Your code: \`${code}\`\nUse \`/redeem\` to claim it. Come back in 24h for another crate!`
            });
    ...
    // -- Daily check-in streak ---------------------------------------------------
    {
        data: new SlashCommandBuilder()
            .setName("streak")
            .setDescription("Check in daily to build your streak"),
        ...
        async execute(interaction){
            ...
            if(record.lastCheckin && now - record.lastCheckin < oneDay){
                const hoursLeft = Math.ceil((oneDay - (now - record.lastCheckin)) / (60 * 60 * 1000));
                return interaction.reply({
                    content: `⏳ Already checked in today. Come back in ~${hoursLeft}h to keep your **${record.streak || 1}-day streak** alive!`,
                    ephemeral: true
                });
            }

            const brokeStreak = !record.lastCheckin || now - record.lastCheckin > oneDay * 2;
            record.streak = brokeStreak ? 1 : (record.streak || 0) + 1;
            record.lastCheckin = now;
            users[interaction.user.id] = record;
            usersDB.write(users);

            const milestones = { 3: "5% off code!", 7: "10% off code!", 14: "20% off code!", 30: "35% off code!" };
            let rewardLine = "Keep checking in daily to build your streak!";

            if(milestones[record.streak]){
                const percent = { 3: 5, 7: 10, 14: 20, 30: 35 }[record.streak];
                const code = `STREAK${record.streak}-${interaction.user.username.slice(0, 4).toUpperCase()}${Math.floor(100 + Math.random() * 900)}`;
                const discounts = discountsDB.read();
                discounts[code] = { percent, usesLeft: 1, createdBy: "streak" };
                discountsDB.write(discounts);
                rewardLine = `🎉 **${record.streak}-day milestone!** Here's a reward code: \`${code}\` (${percent}% off) — use \`/redeem\`!`;
            }

            const fireCount = Math.min(record.streak, 10);
            const embed = brandEmbed({
                title: "🔥 Daily Streak",
                description: `${"🔥".repeat(fireCount)}${fireCount === 0 ? "🔥" : ""}\n\n## Day ${record.streak}\n\n${rewardLine}`
            });
    ...
    // -- 1v1 coinflip duel for bragging rights -----------------------------------
    {
        data: new SlashCommandBuilder()
            .setName("duel")
            .setDescription("Challenge someone to a coinflip duel for bragging rights")
        ...
        async execute(interaction){
            ...
            const embed = brandEmbed({
                ...
                description: `${interaction.user} has challenged ${opponent} to a coinflip duel!\n\nWinner gets bragging rights (and a spot on the duel leaderboard).`,
                thumbnail: false
            });

            const row = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId(`duel_accept_${interaction.user.id}_${opponent.id}`).setLabel("Accept").setEmoji("⚔️").setStyle(ButtonStyle.Success),
                new ButtonBuilder().setCustomId(`duel_decline_${interaction.user.id}_${opponent.id}`).setLabel("Decline").setEmoji("🏳️").setStyle(ButtonStyle.Danger)
            );
    ...
    // -- Staff: post a styled feature roadmap/timeline ----------------------------
    {
        data: new SlashCommandBuilder()
            .setName("roadmap")
            .setDescription("Staff: post an upcoming features roadmap")
            .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
            .addStringOption(o => o.setName("item1").setDescription("e.g. ✅ New payment method").setRequired(true))
            .addStringOption(o => o.setName("item2").setDescription("e.g. 🚧 Faster nuke turnaround").setRequired(false))
            .addStringOption(o => o.setName("item3").setDescription("e.g. 🔜 Loyalty program").setRequired(false))
            .addStringOption(o => o.setName("item4").setDescription("e.g. 🔜 New game support").setRequired(false))
            .addStringOption(o => o.setName("item5").setDescription("e.g. 🔜 Referral rewards v2").setRequired(false)),
        ...
        async execute(interaction){
            ...
            const items = [1, 2, 3, 4, 5].map(n => interaction.options.getString(`item${n}`)).filter(Boolean);

            const embed = brandEmbed({
                title: `🗺️ ${CONFIG.BRAND_NAME} — Roadmap`,
                description: "Here's what's shipped, in progress, and coming up:\n\n" +
                    items.map(i => `▸ ${i}`).join("\n")
            });
    ...
    // -- Fun "server hype" stock-style ticker -------------------------------------
    {
        data: new SlashCommandBuilder()
            .setName("hypemeter")
            .setDescription("See how hyped the server is today vs yesterday"),
        ...
        async execute(interaction){
            ...
            const today = stats.entries.filter(e => now - new Date(e.at).getTime() < oneDay).length;
            const yesterday = stats.entries.filter(e => {
                const age = now - new Date(e.at).getTime();
                return age >= oneDay && age < oneDay * 2;
            }).length;

            const change = today - yesterday;
            const percent = yesterday > 0 ? Math.round((change / yesterday) * 100) : (today > 0 ? 100 : 0);
            const arrow = change > 0 ? "📈" : change < 0 ? "📉" : "➡️";
            const color = change > 0 ? "#43B581" : change < 0 ? "#FF0000" : "#F6821F";

            const embed = brandEmbed({
                title: `${arrow} ${CONFIG.BRAND_NAME} Hype Ticker`,
                color,
                thumbnail: false,
                description:
                    `## $${CONFIG.BRAND_NAME.replace(/\s/g, "").toUpperCase().slice(0, 6)} ${change >= 0 ? "+" : ""}${percent}%\n\n` +
                    `${arrow} Vouches Today: **${today}**\n` +
                    `📅 Vouches Yesterday: **${yesterday}**\n` +
                    `📊 All-Time Total: **${stats.totalVouches}**`
            });
    ...
    // -- Personal achievements grid -------------------------------------------------
    {
        ...
        async execute(interaction){
            ...
            const referrals = referralsDB.read();
            const referralCount = Object.values(referrals).filter(r => r.owner === target.id).reduce((sum, r) => sum + (r.uses?.length || 0), 0);

            const achievements = [
                { name: "First Vouch", emoji: "🥇", unlocked: (record.vouches || 0) >= 1 },
                { name: "5 Vouches", emoji: "⭐", unlocked: (record.vouches || 0) >= 5 },
                { name: "10 Vouches", emoji: "🌟", unlocked: (record.vouches || 0) >= 10 },
                { name: "3-Day Streak", emoji: "🔥", unlocked: (record.streak || 0) >= 3 },
                { name: "7-Day Streak", emoji: "🔥", unlocked: (record.streak || 0) >= 7 },
                { name: "Duel Winner", emoji: "⚔️", unlocked: (record.duelWins || 0) >= 1 },
                { name: "Duel Champion (5 wins)", emoji: "🏆", unlocked: (record.duelWins || 0) >= 5 },
                { name: "Referred a Friend", emoji: "🤝", unlocked: referralCount >= 1 },
                { name: "Crate Collector", emoji: "🎁", unlocked: !!record.lastCrateOpened }
            ];

            const unlockedCount = achievements.filter(a => a.unlocked).length;
            const lines = achievements.map(a => `${a.unlocked ? "✅" : "🔒"} ${a.emoji} ${a.name}`).join("\n");
    ...
    // -- Daily loyalty points (separate lightweight track from /streak) -------
    {
        data: new SlashCommandBuilder()
            .setName("dailyreward")
            .setDescription("Claim your daily loyalty points"),
        ...
        async execute(interaction){
            ...
            if(record.lastDailyReward && now - record.lastDailyReward < oneDay){
                const hoursLeft = Math.ceil((oneDay - (now - record.lastDailyReward)) / (60 * 60 * 1000));
                return interaction.reply({ content: `⏳ Already claimed today. Come back in ~${hoursLeft}h.`, ephemeral: true });
            }

            const earned = 10;
            record.points = (record.points || 0) + earned;
            record.lastDailyReward = now;
            users[interaction.user.id] = record;
            usersDB.write(users);

            const embed = brandEmbed({
                title: "💰 Daily Reward Claimed!",
                description: `You earned **${earned} loyalty points**!\n\n🏦 Total balance: **${record.points} points**`
            });
    ...
    // -- Referral leaderboard (separate from the vouch leaderboard) -----------
    {
        data: new SlashCommandBuilder()
            ...
            .setDescription("See who's referred the most people"),
        ...
        async execute(interaction){
            ...
            for(const data of Object.values(referrals)){
                counts[data.owner] = (counts[data.owner] || 0) + data.uses.length;
            }

            const sorted = Object.entries(counts).filter(([, n]) => n > 0).sort((a, b) => b[1] - a[1]).slice(0, 10);

            const medals = ["🥇", "🥈", "🥉"];
            const lines = sorted.length
                ? sorted.map(([id, n], i) => `${medals[i] || `**${i + 1}.**`} <@${id}> — ${n} referral${n === 1 ? "" : "s"}`).join("\n")
                : "No referrals yet - be the first with `/referral`!";
    ...
    // -- Manually re-post a random past vouch to keep a channel lively --------
    {
        data: new SlashCommandBuilder()
            .setName("socialproof")
            .setDescription("Staff: repost a random past vouch")
            .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),
        ...
        async execute(interaction){
            ...
            const stats = vouchesDB.read();
            if(!stats.entries.length){
                return interaction.reply({ content: "❌ No vouches on file yet.", ephemeral: true });
            }
            ...
            const embed = new EmbedBuilder()
                .setColor("#FFD700")
                .setTitle("💬 Customer Spotlight")
                .setDescription(`> ${pick.comment.replace(/\n/g, "\n> ")}\n\n— <@${pick.user}>`)
                .setFooter({ text: `${CONFIG.BRAND_NAME} • Real customer feedback` })
                .setTimestamp();
    ...
    // -- Check your XP/level ------------------------------------------------------
    {
        data: new SlashCommandBuilder()
            .setName("level")
            .setDescription("Check your activity level")
            .addUserOption(o => o.setName("user").setDescription("Check someone else instead").setRequired(false)),
        ...
        async execute(interaction){
            ...
            const target = interaction.options.getUser("user") || interaction.user;
            const users = usersDB.read();
            const record = users[target.id] || { xp: 0 };
            const xp = record.xp || 0;
            const level = Math.floor(0.1 * Math.sqrt(xp));
            const xpForNextLevel = Math.pow((level + 1) / 0.1, 2);
            const xpForThisLevel = Math.pow(level / 0.1, 2);
            const progress = Math.min(1, Math.max(0, (xp - xpForThisLevel) / (xpForNextLevel - xpForThisLevel)));
            const filled = Math.round(progress * 14);
            const bar = "█".repeat(filled) + "░".repeat(14 - filled);

            const embed = brandEmbed({
                title: `📈 ${target.username}'s Level`,
                description: `## Level ${level}\n\n${bar}\n${xp} / ${Math.round(xpForNextLevel)} XP`
            });
...
const buttonHandlers = {
    ...
    async buy_custom(interaction){
        ...
        if(!panel){
            return interaction.reply({ content: "❌ This panel's data is missing - ask staff to recreate it with `/custompanel`.", ephemeral: true });
        }

        await createTicketFor(interaction, slug, panel.title);
    },
    ...
    // -- Duel challenge accept/decline (customId encodes challenger + opponent
    // IDs, e.g. "duel_accept_123_456" - bot.js routes these here via prefix match)
    async duel_accept(interaction){

        const [, , challengerId, opponentId] = interaction.customId.split("_");

        if(interaction.user.id !== opponentId){
            return interaction.reply({ content: "❌ This duel challenge isn't for you.", ephemeral: true });
        }
        ...
        const users = usersDB.read();
        users[winnerId] = users[winnerId] || { vouches: 0 };
        users[winnerId].duelWins = (users[winnerId].duelWins || 0) + 1;
        users[loserId] = users[loserId] || { vouches: 0 };
        users[loserId].duelLosses = (users[loserId].duelLosses || 0) + 1;
        usersDB.write(users);
        ...
        const embed = brandEmbed({
            ...
            color: "#FFD700",
            thumbnail: false,
            description: `🪙 The coin has spoken...\n\n## 🏆 <@${winnerId}> wins!\n\nBetter luck next time, <@${loserId}>.`
        });
    ...
    async duel_decline(interaction){
        ...
        await interaction.update({
            content: null,
            embeds: [brandEmbed({ title: "🏳️ Duel Declined", description: `<@${opponentId}> backed down from <@${challengerId}>'s challenge.` })],
            components: []
        });
...
async function createTicketFor(interaction, choice, customLabel = null){
    ...
    try{
        await channel.send({ content: `${interaction.user}`, embeds: [embed], components: [row, progressRow, paymentRow] });
    }catch(err){
...
const selectHandlers = {
    ...
    async thriller_select(interaction){
        ...
        if(!panel){
            return interaction.reply({ content: "❌ This panel's data is missing - ask staff to recreate it with `/thrillerpanel`.", ephemeral: true });
        }

        const value = interaction.values[0];
        const product = panel.products.find(p => p.value === value);

        await createTicketFor(interaction, value, product?.label || value);
    },
...
const modalHandlers = {
    ...
    async custompanel_form(interaction){
        ...
        const slug = interaction.fields.getTextInputValue("cp_slug").toLowerCase().replace(/[^a-z0-9_-]/g, "");
...
const xpCooldowns = new Map(); // userId -> timestamp of last XP award, 60s cooldown per message
...
async function handleXpMessage(message){
    ...
    const users = usersDB.read();
    const record = users[message.author.id] || { vouches: 0 };
    const oldLevel = Math.floor(0.1 * Math.sqrt(record.xp || 0));

    record.xp = (record.xp || 0) + Math.floor(15 + Math.random() * 10);
    users[message.author.id] = record;
    usersDB.write(users);

    const newLevel = Math.floor(0.1 * Math.sqrt(record.xp));
    if(newLevel > oldLevel){
        message.channel.send({
            content: `🎉 ${message.author} just reached **Level ${newLevel}**!`
        }).catch(() => {});
    }
}
