...
const slashCommands = [
    ...
    // -- Build-your-own ticket panel (any title/price/features/image) --------
    {
        data: new SlashCommandBuilder()
            .setName("custompanel")
            .setDescription("Staff: build a custom ticket panel with your own title, price, features & image")
            .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
            .addStringOption(o => o.setName("title").setDescription("Panel title (e.g. Damascus Camo)").setRequired(true))
            .addStringOption(o => o.setName("slug").setDescription("Short unique ID, no spaces (e.g. damascus)").setRequired(true))
            .addStringOption(o => o.setName("price").setDescription("Price line (e.g. FROM $25)").setRequired(true))
            .addStringOption(o => o.setName("features").setDescription("Bullet points, separated by | (pipe)").setRequired(true))
            .addAttachmentOption(o => o.setName("image").setDescription("Image for the bottom of the panel").setRequired(true))
            .addStringOption(o => o.setName("button_label").setDescription("Button text (default: Open Ticket)").setRequired(false)),

        async execute(interaction){

            const title = interaction.options.getString("title");
            const slug = interaction.options.getString("slug").toLowerCase().replace(/[^a-z0-9_-]/g, "");
            const price = interaction.options.getString("price");
            const features = interaction.options.getString("features").split("|").map(f => f.trim()).filter(Boolean);
            const image = interaction.options.getAttachment("image");
            const buttonLabel = interaction.options.getString("button_label") || "Open Ticket";

            if(!slug){
                return interaction.reply({ content: "❌ That slug isn't valid - use letters, numbers, dashes or underscores only.", ephemeral: true });
            }
