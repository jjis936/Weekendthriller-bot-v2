...
const slashCommands = [
    ...
    // 9 FIRE COMMANDS - UNIQUE NAMES
    { data: new SlashCommandBuilder().setName("nexus").setDescription("💎 Enter the tier nexus"), execute: async (i) => { const embeds = []; const tiers = [{ name: "🥉 BRONZE", perks: "Standard support" }, { name: "🥈 SILVER", perks: "5% off" }, { name: "🥇 GOLD", perks: "10% off" }, { name: "💜 PLATINUM", perks: "15% off" }, { name: "👑 APEX", perks: "20% off" }, { name: "🔓 ECLIPSE", perks: "Custom" }]; for (const t of tiers) { embeds.push(new EmbedBuilder().setColor("#9D00FF").setTitle(`${t.name} TIER`).addField("💰", t.perks, true).setTimestamp()); } await i.reply({ embeds }); } },
    { data: new SlashCommandBuilder().setName("codex").setDescription("🔓 Unlock the service codex"), execute: async (i) => { const e = new EmbedBuilder().setColor("#9D00FF").setTitle("🔓 CODEX").addField("🚀 WZ", "$15-$65", true).addField("🎮 MP", "$10-$50", true).addField("🎨 CAMO", "$5-$75", true).addField("📱 SMS", "$2-$75", true).addField("👾 OPS", "$8-$80", true).addField("💎 VIP", "$25-$180", true).setFooter({ text: "CashApp $EvanJoyner8" }).setTimestamp(); await i.reply({ embeds: [e] }); } },
    { data: new SlashCommandBuilder().setName("crimson").setDescription("💳 Crimson payment gateway"), execute: async (i) => { const e = new EmbedBuilder().setColor("#FF00FF").setTitle("💳 CRIMSON GATEWAY").addField("💰", "`$EvanJoyner8`", true).addField("⚡", "Instant", true).setFooter({ text: "VERIFIED SECURE" }).setTimestamp(); const r = new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel("💰 Send").setStyle(ButtonStyle.Link).setURL("https://cash.app/$EvanJoyner8")); await i.reply({ embeds: [e], components: [r], ephemeral: true }); } },
    { data: new SlashCommandBuilder().setName("inferno").setDescription("🎁 Inferno bundle collection"), execute: async (i) => { const embeds = [new EmbedBuilder().setColor("#9D00FF").setTitle("🎁 STARTER").addField("📦", "$50 (Save $15)", false).setTimestamp(), new EmbedBuilder().setColor("#FF00FF").setTitle("💜 ELITE").addField("📦", "$120 (Save $35)", false).setTimestamp(), new EmbedBuilder().setColor("#9D00FF").setTitle("👑 APEX").addField("📦", "$250 (Save $70)", false).setTimestamp(), new EmbedBuilder().setColor("#FF00FF").setTitle("🔓 INFERNO").addField("📦", "CUSTOM", false).setTimestamp()]; await i.reply({ embeds }); } },
    { data: new SlashCommandBuilder().setName("sanctum").setDescription("🔓 Enter the sanctum vault"), execute: async (i) => { const e = new EmbedBuilder().setColor("#9D00FF").setTitle("💀 SANCTUM 💀").setDescription("🔓 UNDERGROUND | ⚡ INSTANT | 💰 `$EvanJoyner8`").setTimestamp(); const r = new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel("💳 Access").setStyle(ButtonStyle.Link).setURL("https://cash.app/$EvanJoyner8")); await i.reply({ embeds: [e], components: [r] }); } },
    { data: new SlashCommandBuilder().setName("fortress").setDescription("🔒 Fortress security protocols"), execute: async (i) => { const e = new EmbedBuilder().setColor("#9D00FF").setTitle("🔒 FORTRESS 🔒").addField("🛡️ LOCKED DOWN", "✅ Encryption\n✅ VPN\n✅ 2FA", false).addField("📊 DATA SEALED", "✅ Never shared\n✅ Auto-purge 30d\n✅ GDPR locked", false).setFooter({ text: "MAXIMUM PROTECTION" }).setTimestamp(); await i.reply({ embeds: [e] }); } },
    { data: new SlashCommandBuilder().setName("legends").setDescription("⭐ Legends testimonial hall"), execute: async (i) => { const e = new EmbedBuilder().setColor("#FF00FF").setTitle("⭐ LEGENDS ⭐").setDescription("500+ 5-STAR | 98.7% ELITE").addField("@Shadow", "\"Insane service\"", false).addField("@Ghost", "\"Legendary\"", false).addField("@Apex", "\"Perfect\"", false).setFooter({ text: "HALL OF FAME" }).setTimestamp(); await i.reply({ embeds: [e] }); } },
    { data: new SlashCommandBuilder().setName("oracle").setDescription("❓ Oracle's knowledge base"), execute: async (i) => { const e = new EmbedBuilder().setColor("#9D00FF").setTitle("❓ ORACLE ❓").addField("Safe?", "✅ Bulletproof", false).addField("Fast?", "✅ Lightning", false).addField("Refund?", "✅ Guaranteed", false).addField("Pay?", "✅ CashApp `$EvanJoyner8`", false).setFooter({ text: "WISDOM UNLOCKED" }).setTimestamp(); await i.reply({ embeds: [e] }); } },
    { data: new SlashCommandBuilder().setName("crown").setDescription("💎 Crown VIP privileges"), execute: async (i) => { const embeds = [new EmbedBuilder().setColor("#FF00FF").setTitle("💜 CROWN SILVER").addField("🎁", "5% + Priority", false).setTimestamp(), new EmbedBuilder().setColor("#FF00FF").setTitle("🥇 CROWN GOLD").addField("🎁", "10% + VIP", false).setTimestamp(), new EmbedBuilder().setColor("#FF00FF").setTitle("💜 CROWN PLAT").addField("🎁", "15% + Handler", false).setTimestamp(), new EmbedBuilder().setColor("#9D00FF").setTitle("👑 CROWN APEX").addField("🎁", "20% + Private", false).setTimestamp()]; await i.reply({ embeds }); } },
