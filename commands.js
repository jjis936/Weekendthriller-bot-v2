...
const slashCommands = [
    ...
    // 9 FIRE COMMANDS - FIXED
    { data: new SlashCommandBuilder().setName("nexus").setDescription("💎 Enter the tier nexus"), execute: async (interaction) => { const embeds = [new EmbedBuilder().setColor("#9D00FF").setTitle("🥉 BRONZE").addField("Tier", "Entry Level", false).setTimestamp(), new EmbedBuilder().setColor("#9D00FF").setTitle("🥈 SILVER").addField("Tier", "5% off", false).setTimestamp(), new EmbedBuilder().setColor("#9D00FF").setTitle("🥇 GOLD").addField("Tier", "10% off", false).setTimestamp(), new EmbedBuilder().setColor("#FF00FF").setTitle("💜 PLATINUM").addField("Tier", "15% off", false).setTimestamp(), new EmbedBuilder().setColor("#FF00FF").setTitle("👑 APEX").addField("Tier", "20% off", false).setTimestamp(), new EmbedBuilder().setColor("#FF00FF").setTitle("🔓 ECLIPSE").addField("Tier", "Custom", false).setTimestamp()]; await interaction.reply({ embeds }); } },
    { data: new SlashCommandBuilder().setName("codex").setDescription("🔓 Unlock the service codex"), execute: async (interaction) => { const embed = new EmbedBuilder().setColor("#9D00FF").setTitle("🔓 CODEX").addField("🚀 WZ", "$15-$65", true).addField("🎮 MP", "$10-$50", true).addField("🎨 CAMO", "$5-$75", true).addField("📱 SMS", "$2-$75", true).addField("👾 OPS", "$8-$80", true).addField("💎 VIP", "$25-$180", true).setFooter({ text: "CashApp $EvanJoyner8" }).setTimestamp(); await interaction.reply({ embeds: [embed] }); } },
    { data: new SlashCommandBuilder().setName("crimson").setDescription("💳 Crimson payment gateway"), execute: async (interaction) => { const embed = new EmbedBuilder().setColor("#FF00FF").setTitle("💳 CRIMSON GATEWAY").addField("Payment", "`$EvanJoyner8`", true).addField("Speed", "Instant", true).addField("Status", "VERIFIED", false).setFooter({ text: "SECURE TRANSACTION" }).setTimestamp(); const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel("💰 Send Payment").setStyle(ButtonStyle.Link).setURL("https://cash.app/$EvanJoyner8")); await interaction.reply({ embeds: [embed], components: [row], ephemeral: true }); } },
    { data: new SlashCommandBuilder().setName("inferno").setDescription("🎁 Inferno bundle collection"), execute: async (interaction) => { const embeds = [new EmbedBuilder().setColor("#9D00FF").setTitle("🎁 STARTER BUNDLE").addField("Price", "$50 (Save $15)", false).setTimestamp(), new EmbedBuilder().setColor("#FF00FF").setTitle("💜 ELITE BUNDLE").addField("Price", "$120 (Save $35)", false).setTimestamp(), new EmbedBuilder().setColor("#9D00FF").setTitle("👑 APEX BUNDLE").addField("Price", "$250 (Save $70)", false).setTimestamp(), new EmbedBuilder().setColor("#FF00FF").setTitle("🔓 INFERNO BUNDLE").addField("Price", "CUSTOM QUOTE", false).setTimestamp()]; await interaction.reply({ embeds }); } },
    { data: new SlashCommandBuilder().setName("sanctum").setDescription("🔓 Enter the sanctum vault"), execute: async (interaction) => { const embed = new EmbedBuilder().setColor("#9D00FF").setTitle("💀 SANCTUM VAULT 💀").setDescription("🔓 UNDERGROUND ACCESS\n⚡ INSTANT DELIVERY\n💰 CashApp `$EvanJoyner8`").addField("Status", "UNLOCKED", false).setFooter({ text: "VAULT AUTHENTICATED" }).setTimestamp(); const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel("💳 Access Vault").setStyle(ButtonStyle.Link).setURL("https://cash.app/$EvanJoyner8")); await interaction.reply({ embeds: [embed], components: [row] }); } },
    { data: new SlashCommandBuilder().setName("fortress").setDescription("🔒 Fortress security protocols"), execute: async (interaction) => { const embed = new EmbedBuilder().setColor("#9D00FF").setTitle("🔒 FORTRESS SECURITY 🔒").addField("🛡️ PROTECTION", "✅ Encryption\n✅ VPN Routing\n✅ 2FA Support\n✅ Zero Compromise", false).addField("📊 DATA SECURITY", "✅ Never Shared\n✅ Auto-Delete 30d\n✅ GDPR Compliant\n✅ Encrypted Only", false).addField("📈 TRACK RECORD", "✅ 2,800+ Customers\n✅ Zero Bans (30d)\n✅ 99.8% Safety\n✅ 500+ Reviews", false).setFooter({ text: "MAXIMUM PROTECTION" }).setTimestamp(); await interaction.reply({ embeds: [embed] }); } },
    { data: new SlashCommandBuilder().setName("legends").setDescription("⭐ Legends testimonial hall"), execute: async (interaction) => { const embed = new EmbedBuilder().setColor("#FF00FF").setTitle("⭐ LEGENDS HALL ⭐").setDescription("500+ 5-STAR REVIEWS\n98.7% ELITE SATISFACTION").addField("@Shadow", "\"50 wins in 2 days. Insane!\"", false).addField("@Ghost", "\"Fast, reliable, professional.\"", false).addField("@Apex", "\"Support fixed issue in 5min.\"", false).addField("@Titan", "\"6 months, 25 purchases, perfect.\"", false).setFooter({ text: "HALL OF FAME" }).setTimestamp(); await interaction.reply({ embeds: [embed] }); } },
    { data: new SlashCommandBuilder().setName("oracle").setDescription("❓ Oracle's knowledge base"), execute: async (interaction) => { const embed = new EmbedBuilder().setColor("#9D00FF").setTitle("❓ ORACLE WISDOM ❓").addField("Q: Is account safe?", "✅ YES. Zero bans. Industry security.", false).addField("Q: How fast is delivery?", "✅ INSTANT to 24h. Guaranteed.", false).addField("Q: Money-back guarantee?", "✅ YES. 100% within 48h.", false).addField("Q: Will I get banned?", "✅ NO. Zero bans reported. Expert handling.", false).addField("Q: How to pay?", "✅ CashApp `$EvanJoyner8`. Instant. Secure.", false).setFooter({ text: "WISDOM UNLOCKED" }).setTimestamp(); await interaction.reply({ embeds: [embed] }); } },
    { data: new SlashCommandBuilder().setName("crown").setDescription("💎 Crown VIP privileges"), execute: async (interaction) => { const embeds = [new EmbedBuilder().setColor("#FF00FF").setTitle("💜 CROWN SILVER").addField("Benefits", "✅ 5% off\n✅ Priority Support\n✅ VIP Badge", false).setTimestamp(), new EmbedBuilder().setColor("#FF00FF").setTitle("🥇 CROWN GOLD").addField("Benefits", "✅ 10% off\n✅ VIP Support (5min)\n✅ Custom Packages", false).setTimestamp(), new EmbedBuilder().setColor("#FF00FF").setTitle("💜 CROWN PLATINUM").addField("Benefits", "✅ 15% off\n✅ 24/7 Handler\n✅ Monthly $100 Free", false).setTimestamp(), new EmbedBuilder().setColor("#9D00FF").setTitle("👑 CROWN APEX").addField("Benefits", "✅ 20% off ALL\n✅ Private Network\n✅ Lifetime Recovery", false).setTimestamp()]; await interaction.reply({ embeds }); } },
