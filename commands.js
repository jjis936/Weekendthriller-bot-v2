...
const modalHandlers = {
    ...
    // -- Account listing: after title/description/price, collect images then post
    async account_listing_form(interaction){
        ...
        await interaction.reply({
            content: "📸 Now send **1-5 screenshots** of the account right here in this channel (as attachments). Type `done` when finished, or I'll wrap up automatically after 5 minutes.",
            ephemeral: true
        });
        ...
        collector.on("end", async () => {
            ...
            if(imageUrls.length === 0){
                await interaction.followUp({
                    content: "❌ No images received in time, so the listing was cancelled. Run `/sellaccount` again when you're ready.",
                    ephemeral: true
                }).catch(() => {});
                return;
            }
            ...
            if(!postChannel){
                await interaction.followUp({
                    content: "❌ Couldn't find the listing channel - contact staff so they can check ACCOUNT_LISTING_POST_CHANNEL_ID.",
                    ephemeral: true
                }).catch(() => {});
                return;
            }
            ...
            try{
                ...
                await interaction.followUp({
                    content: `✅ Your listing is up in ${postChannel}!`,
                    ephemeral: true
                });
            }catch(err){
                console.log(`[sellaccount] could not post listing: ${err.message}`);
                await interaction.followUp({
                    content: `❌ Couldn't post your listing (\`${err.message}\`).`,
                    ephemeral: true
                }).catch(() => {});
            }
