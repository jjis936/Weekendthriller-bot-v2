...
const CONFIG = {
    ...
    // Channels where NO images/attachments are allowed at all (comma-separated
    // channel IDs in the env var). Great for "chat only" channels where scam
    // screenshots (fake giveaways, phishing sites, etc.) keep getting posted.
    TEXT_ONLY_CHANNEL_IDS: (process.env.TEXT_ONLY_CHANNEL_IDS || "").split(",").map(s => s.trim()).filter(Boolean),

    // Known scam domains to auto-delete on sight, server-wide, regardless of
    // channel. Add more by setting BLOCKED_DOMAINS as a comma-separated env
    // var - it's merged with this default list, not replaced.
    BLOCKED_DOMAINS: [
        "winleg.com",
        ...(process.env.BLOCKED_DOMAINS || "").split(",").map(s => s.trim()).filter(Boolean)
    ],

    // Channels where NO images/videos are allowed at all (staff exempt) - IDs
    // comma-separated in the env var, e.g. "111111,222222"
    NO_MEDIA_CHANNEL_IDS: (process.env.NO_MEDIA_CHANNEL_IDS || "").split(",").map(s => s.trim()).filter(Boolean),
    ...
    KINGPIN_ROLE_ID: process.env.KINGPIN_ROLE_ID || null, // optional - auto-assigned to whoever holds the Kingpin title
...
// -- /heist: live community event embed builder ------------------------------
function buildHeistEmbed({ participantCount, secondsLeft, ended, winners, prize }){

    if(ended){
        const embed = new EmbedBuilder()
            .setColor("#FFD700")
            .setTitle("🏴‍☠️ HEIST COMPLETE")
            .setDescription(
                "```\n💰 THE VAULT HAS BEEN CRACKED 💰\n```\n" +
                (winners.length
                    ? `**${winners.length}** crew member${winners.length > 1 ? "s" : ""} scored the haul:\n\n` +
                      winners.map(w => `${w.emoji} <@${w.userId}> — **${w.percent}% off** \`${w.code}\``).join("\n")
                    : "😬 The crew was too small - nobody scored this time. Bigger crew next time!"
                ) +
                "\n\n▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"
            )
            .setFooter({ text: `${CONFIG.BRAND_NAME} • ${participantCount} crew member${participantCount === 1 ? "" : "s"} joined` })
            .setTimestamp();
        return embed;
    }
    ...
    const embed = new EmbedBuilder()
        .setColor("#FF4500")
        .setTitle("🏴‍☠️ A HEIST IS GOING DOWN")
        .setDescription(
            "```\n🚨 CREW UP BEFORE TIME RUNS OUT 🚨\n```\n" +
            `Tap **Join the Crew** to get in. When the timer hits zero, we crack the vault and hand out loot to a lucky few.\n\n` +
            `⏳ **Time left:** ${secondsLeft}s\n👥 **Crew size:** ${participantCount}\n\n` +
            "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"
        )
        .setFooter({ text: `${CONFIG.BRAND_NAME} • Bigger crews = better odds` })
        .setTimestamp();
...
function buildVouchResultEmbed({ user, rating, comment, totalVouches, anonymous = false, ghostTag = null }){
    ...
    if(anonymous){
        embed.setAuthor({ name: ghostTag ? `🕶️ Ghost: ${ghostTag}` : "🕶️ Anonymous Customer" });
    }else{
...
const heistDB = db("heists", {}); // { messageId: { channelId, endTime, participants: [], ended } }
const kingpinDB = db("kingpin", null); // { userId, crownedAt, crownedBy, reason } | null
const textOnlyChannelsDB = db("textonlychannels", []); // [channelId, channelId, ...] - toggled live via /textonly
...
// ============================================================================
// EASY-EDIT CONTENT for the /nexus, /codex, /inferno, /sanctum, /fortress,
// /oracle, and /crown "fire" commands below. Change any wording, prices, or
// values here and it updates everywhere that catalog is used - no need to
// touch the embed-building code itself.
// ============================================================================

// /nexus - 6 tier showcase
const NEXUS_TIERS = [
    { emoji: "🥉", name: "BRONZE", requirement: "Entry Level", perk: "Standard pricing, standard queue" },
    { emoji: "🥈", name: "SILVER", requirement: "3+ orders", perk: "5% off all services" },
    { emoji: "🥇", name: "GOLD", requirement: "7+ orders", perk: "10% off + priority queue" },
    { emoji: "💜", name: "PLATINUM", requirement: "15+ orders", perk: "15% off + dedicated handler" },
    { emoji: "👑", name: "APEX", requirement: "30+ orders", perk: "20% off + skip-the-line priority" },
    { emoji: "🔓", name: "ECLIPSE", requirement: "Invite only", perk: "Custom pricing on everything" }
];

// /codex - service pricing categories
const CODEX_CATEGORIES = [
    { emoji: "🚀", name: "Warzone Boosting", range: "$15 - $65" },
    { emoji: "🎮", name: "Multiplayer Boosting", range: "$10 - $50" },
    { emoji: "🎨", name: "Camo Grinding", range: "$5 - $75" },
    { emoji: "📱", name: "SMS / Number Rental", range: "$2 - $75" },
    { emoji: "👾", name: "Operators & Unlocks", range: "$8 - $80" },
    { emoji: "💎", name: "VIP / Custom Packages", range: "$25 - $180" }
];

// /inferno - 4 bundle deals
const INFERNO_BUNDLES = [
    { emoji: "🎁", name: "STARTER BUNDLE", price: "$50", savings: "Save $15", includes: "1x Rank Boost + 1x Camo Unlock" },
    { emoji: "💜", name: "ELITE BUNDLE", price: "$120", savings: "Save $35", includes: "1x Nuke + Full Camo Path + Operator" },
    { emoji: "👑", name: "APEX BUNDLE", price: "$250", savings: "Save $70", includes: "Full Rank Boost + 2x Nuke + VIP Handling" },
    { emoji: "🔓", name: "INFERNO BUNDLE", price: "Custom Quote", savings: "Biggest savings tier", includes: "Everything, tailored to you - open a ticket" }
];

// /fortress - security protocols
const FORTRESS_PROTOCOLS = [
    { emoji: "🛡️", name: "PROTECTION", points: ["Encrypted handling throughout", "No password requests outside a staff-claimed ticket", "2FA-safe methods only", "Zero-compromise policy on account safety"] },
    { emoji: "📊", name: "DATA SECURITY", points: ["Your info is never shared or sold", "Ticket transcripts auto-managed, not left exposed", "Payment info never stored by us", "CashApp is the only manual step - everything else is automated"] },
    { emoji: "📈", name: "TRACK RECORD", points: ["Real, verifiable vouches in our vouch channel", "Staff-claimed tickets only - no anonymous handling", "Transparent order tracking with `/trackorder`", "Live support via `/support` and `/ask`"] }
];

// /crown - 4 VIP tiers
const CROWN_TIERS = [
    { emoji: "💜", name: "CROWN SILVER", benefits: ["5% off all services", "Priority support queue", "VIP role & badge"] },
    { emoji: "🥇", name: "CROWN GOLD", benefits: ["10% off all services", "Faster staff response", "Access to custom packages"] },
    { emoji: "💎", name: "CROWN PLATINUM", benefits: ["15% off all services", "Dedicated order handler", "First access to restocks"] },
    { emoji: "👑", name: "CROWN APEX", benefits: ["20% off everything", "Direct staff line", "Highest priority on every order"] }
];

// /wraith - random codename pool for the ghost identity system
const WRAITH_NAMES = [
    "Nightshade", "Voidwalker", "Obsidian", "Phantom", "Wraithborn", "Duskrunner",
    "Silentblade", "Ashwolf", "Grimhollow", "Nocturne", "Shadowfen", "Blackfrost",
    "Ravencall", "Direwatch", "Hollowmoon", "Ironveil", "Crimsonfang", "Emberwraith"
];
...
const slashCommands = [
    ...
    // -- Waitlist: join a service waitlist, or (staff) ping everyone on it ----
    {
        data: new SlashCommandBuilder()
            .setName("waitlist")
            .setDescription("Join a service waitlist, or (staff) notify everyone on one")
            .addSubcommand(sc => sc.setName("join").setDescription("Join the waitlist for a service")
                .addStringOption(o => o.setName("service").setDescription("Which service (e.g. nuke, camo-grinding)").setRequired(true)))
            .addSubcommand(sc => sc.setName("ping").setDescription("Staff: notify everyone waitlisted for a service")
    ...
    // ==========================================================================
    // 9 "FIRE" BLACK-MARKET THEMED COMMANDS
    // Content for these lives in the NEXUS_TIERS / CODEX_CATEGORIES /
    // INFERNO_BUNDLES / FORTRESS_PROTOCOLS / CROWN_TIERS catalogs near the top
    // of this file - edit those to change wording/pricing without touching
    // any of the code below.
    // ==========================================================================

    // -- /nexus: 6-tier customer status ladder ---------------------------------
    {
        data: new SlashCommandBuilder()
            .setName("nexus")
            .setDescription("💎 Enter the tier nexus"),
        ...
        async execute(interaction){
            ...
            const embed = new EmbedBuilder()
                ...
                .setTitle("💎 THE NEXUS")
                .setDescription(
                    "```\n⚡ 6-TIER STATUS LADDER ⚡\n```\n" +
                    "Every order pushes you up the ladder. Higher tiers unlock bigger discounts and faster priority.\n\n" +
                    "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"
                )
                .addFields(NEXUS_TIERS.map(t => ({
                    name: `${t.emoji} ${t.name}`,
                    value: `**Requirement:** ${t.requirement}\n**Perk:** ${t.perk}`,
                    inline: true
                })))
                .setFooter({ text: `${CONFIG.BRAND_NAME} • Climb the Nexus` })
                .setTimestamp();
    ...
    // -- /codex: service pricing reference --------------------------------------
    {
        data: new SlashCommandBuilder()
            .setName("codex")
            .setDescription("🔓 Unlock the service codex"),
        ...
        async execute(interaction){
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(
                    ...
                    "Everything we offer, all in one place. Open a ticket for an exact quote on any category.\n\n" +
                    "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"
                )
                .addFields(CODEX_CATEGORIES.map(c => ({
                    name: `${c.emoji} ${c.name}`,
                    value: c.range,
                    inline: true
                })))
                .setFooter({ text: `${CONFIG.BRAND_NAME} • CashApp ${CONFIG.CASHAPP_TAG}` })
                .setTimestamp();
    ...
    // -- /crimson: CashApp payment gateway --------------------------------------
    {
        data: new SlashCommandBuilder()
            .setName("crimson")
            .setDescription("💳 Crimson payment gateway"),

        async execute(interaction){

            const { embed, row } = buildCashAppPaymentCard();
            embed.setTitle("💳 CRIMSON GATEWAY").setColor("#FF00FF");

            const linkRow = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setLabel("💰 Open Cash App").setStyle(ButtonStyle.Link).setURL(`https://cash.app/${CONFIG.CASHAPP_TAG}`)
            );

            await interaction.reply({ embeds: [embed], components: [linkRow, row], ephemeral: true });
        }
    },

    // -- /inferno: bundle deals ---------------------------------------------------
    {
        data: new SlashCommandBuilder()
            .setName("inferno")
            .setDescription("🎁 Inferno bundle collection"),

        async execute(interaction){

            const embed = new EmbedBuilder()
                .setColor("#FF4500")
                .setTitle("🔥 THE INFERNO")
                .setDescription(
                    "```\n🎁 BUNDLE COLLECTION 🎁\n```\n" +
                    "Stack services together and save. Bigger bundle, bigger discount.\n\n" +
                    "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"
                )
                .addFields(INFERNO_BUNDLES.map(b => ({
                    name: `${b.emoji} ${b.name} — ${b.price}`,
                    value: `**${b.savings}**\n${b.includes}`,
                    inline: false
                })))
                .setFooter({ text: `${CONFIG.BRAND_NAME} • Open a ticket to lock in a bundle` })
                .setTimestamp();
    ...
    // -- /sanctum: black market vault - opens a real ticket ----------------------
    {
        data: new SlashCommandBuilder()
            .setName("sanctum")
            .setDescription("🔓 Enter the sanctum vault"),

        async execute(interaction){

            const embed = new EmbedBuilder()
                .setColor("#9D00FF")
                .setTitle("💀 SANCTUM VAULT 💀")
                .setDescription(
                    "```\n🔓 UNDERGROUND ACCESS 🔓\n```\n" +
                    "⚡ Instant delivery on unlock\n" +
                    "🔒 Discreet, staff-handled process\n" +
                    `💰 Payment via CashApp \`${CONFIG.CASHAPP_TAG}\`\n\n` +
                    "Tap below to open a private vault access ticket.\n\n" +
                    "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"
                )
                .addFields({ name: "Status", value: "🟢 UNLOCKED - Vault is open for access", inline: false })
                .setFooter({ text: `${CONFIG.BRAND_NAME} • Vault Authenticated` })
                .setTimestamp();

            const row = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId("sanctum_access").setLabel("Access Vault").setEmoji("💳").setStyle(ButtonStyle.Danger)
            );
    ...
    // -- /fortress: security protocols -------------------------------------------
    {
        data: new SlashCommandBuilder()
            .setName("fortress")
            .setDescription("🔒 Fortress security protocols"),

        async execute(interaction){

            const embed = new EmbedBuilder()
                .setColor("#9D00FF")
                .setTitle("🔒 FORTRESS SECURITY 🔒")
                .setDescription(
                    "```\n🛡️ FULL SECURITY PROTOCOLS 🛡️\n```\n▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"
                )
                .addFields(FORTRESS_PROTOCOLS.map(p => ({
                    name: `${p.emoji} ${p.name}`,
                    value: p.points.map(pt => `✅ ${pt}`).join("\n"),
                    inline: false
                })))
                .setFooter({ text: `${CONFIG.BRAND_NAME} • Maximum Protection` })
                .setTimestamp();
    ...
    // -- /legends: real customer testimonials from actual vouch data -----------
    {
        data: new SlashCommandBuilder()
            .setName("legends")
            .setDescription("⭐ Legends testimonial hall"),

        async execute(interaction){

            const stats = vouchesDB.read();
            const avg = stats.totalVouches ? (stats.ratingSum / stats.totalVouches).toFixed(1) : "N/A";

            const embed = new EmbedBuilder()
                .setColor("#FF00FF")
                .setTitle("⭐ THE LEGENDS HALL ⭐")
                .setDescription(
                    "```\n🏆 HALL OF FAME 🏆\n```\n" +
                    `**${stats.totalVouches}** total vouches • **${avg}/5** average rating\n\n` +
                    "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"
                )
                .setFooter({ text: `${CONFIG.BRAND_NAME} • Real customers, real results` })
                .setTimestamp();

            if(stats.entries.length === 0){
                embed.addFields({ name: "No vouches yet", value: "Be the first to leave one - check the vouch channel!" });
            }else{
                const featured = [...stats.entries].sort(() => 0.5 - Math.random()).slice(0, 5);
                for(const v of featured){
                    embed.addFields({
                        name: `<@${v.user}>`,
                        value: `"${v.comment.slice(0, 150)}${v.comment.length > 150 ? "..." : ""}"`,
                        inline: false
                    });
                }
            }
    ...
    // -- /oracle: full FAQ reference, all at once --------------------------------
    {
        data: new SlashCommandBuilder()
            .setName("oracle")
            .setDescription("❓ Oracle's knowledge base"),

        async execute(interaction){

            const embed = new EmbedBuilder()
                .setColor("#9D00FF")
                .setTitle("❓ ORACLE WISDOM ❓")
                .setDescription(
                    "```\n🔮 IN-DEPTH KNOWLEDGE BASE 🔮\n```\nEverything you need to know, in one place. Still have questions? Use `/ask` or open a ticket.\n\n▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"
                )
                .addFields(FAQ_CATALOG.map(f => ({
                    name: `${f.emoji} ${f.question}`,
                    value: f.answer,
                    inline: false
                })))
                .setFooter({ text: `${CONFIG.BRAND_NAME} • Wisdom Unlocked` })
                .setTimestamp();
    ...
    // -- /crown: VIP membership tiers ---------------------------------------------
    {
        data: new SlashCommandBuilder()
            .setName("crown")
            .setDescription("💎 Crown VIP privileges"),
        ...
        async execute(interaction){
            ...
            const embed = new EmbedBuilder()
                ...
                .setTitle("👑 THE CROWN 👑")
                .setDescription(
                    "```\n💎 VIP MEMBERSHIP TIERS 💎\n```\nUnlock exclusive perks as a loyal customer.\n\n▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"
                )
                .addFields(CROWN_TIERS.map(t => ({
                    name: `${t.emoji} ${t.name}`,
                    value: t.benefits.map(b => `✅ ${b}`).join("\n"),
                    inline: true
                })))
                .setFooter({ text: `${CONFIG.BRAND_NAME} • Wear the Crown` })
                .setTimestamp();
    ...
    // -- /wraith: ghost identity system - generates a persistent anonymous
    // codename used whenever this customer chooses to post an anonymous vouch --
    {
        data: new SlashCommandBuilder()
            .setName("wraith")
            .setDescription("🕶️ Claim your ghost identity for anonymous vouches"),
        ...
        async execute(interaction){
            ...
            if(record.ghostTag){
                const embed = new EmbedBuilder()
                    .setColor("#2C2F33")
                    .setTitle("🕶️ Your Ghost Identity")
                    .setDescription(
                        "```\n👻 WRAITH PROTOCOL 👻\n```\n" +
                        `You're already known as **Ghost: ${record.ghostTag}**.\n\n` +
                        "This name shows up instead of \"Anonymous Customer\" whenever you post a vouch anonymously.\n\n" +
                        "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"
                    )
                    .setFooter({ text: CONFIG.BRAND_NAME })
                    .setTimestamp();
                ...
                const rerollRow = new ActionRowBuilder().addComponents(
                    new ButtonBuilder().setCustomId("wraith_reroll").setLabel("Reroll Codename").setEmoji("🔁").setStyle(ButtonStyle.Secondary)
                );

                return interaction.reply({ embeds: [embed], components: [rerollRow], ephemeral: true });
            }

            const tag = WRAITH_NAMES[Math.floor(Math.random() * WRAITH_NAMES.length)];
            record.ghostTag = tag;
            ...
            const embed = new EmbedBuilder()
                .setColor("#2C2F33")
                .setTitle("🕶️ Ghost Identity Assigned")
                .setDescription(
                    "```\n👻 WRAITH PROTOCOL ACTIVATED 👻\n```\n" +
                    `You are now known as **Ghost: ${tag}**.\n\n` +
                    "This name will show up instead of \"Anonymous Customer\" whenever you choose to post a vouch anonymously — so you still get recognized without revealing who you are.\n\n" +
                    "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"
                )
                .setFooter({ text: CONFIG.BRAND_NAME })
                .setTimestamp();
    ...
    // -- /obsidian: rotating daily black-market deal, deterministic per day --
    {
        data: new SlashCommandBuilder()
            .setName("obsidian")
            .setDescription("🖤 Reveal today's black market deal"),
        ...
        async execute(interaction){
            ...
            const service = SERVICE_CATALOG[seed % SERVICE_CATALOG.length];
            const discounts = [10, 15, 20, 25, 30];
            const discountPercent = discounts[seed % discounts.length];

            const basePrice = parseFloat(String(service.price).replace(/[^0-9.]/g, "")) || null;
            const dealPrice = basePrice ? `$${(basePrice * (1 - discountPercent / 100)).toFixed(2)}` : "Ask staff for the discounted quote";

            const embed = new EmbedBuilder()
                .setColor("#1a1a1a")
                .setTitle("🖤 THE OBSIDIAN MARKET 🖤")
                .setDescription(
                    "```\n🌑 TODAY'S BLACK MARKET DEAL 🌑\n```\n" +
                    `Resets daily. Today's find:\n\n` +
                    `## ${service.label}\n\n` +
                    `~~${service.price}~~ → **${dealPrice}** (${discountPercent}% off)\n\n` +
                    `⏱️ Turnaround: ${service.turnaround}\n🎁 Includes: ${service.reward}\n\n` +
                    "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n⚠️ Today only - gone at midnight."
                )
                .setFooter({ text: `${CONFIG.BRAND_NAME} • Come back tomorrow for a new deal` })
                .setTimestamp();
    ...
    // -- /blueprint: advanced panel builder - pick a layout + color up front,
    // then a modal for the details, just like /custompanel but way more
    // customizable. Reuses the same buy_custom_<slug> ticket system. --------
    {
        data: new SlashCommandBuilder()
            .setName("blueprint")
            .setDescription("📐 Design a fully custom panel: layout, color, and more")
            .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
            .addStringOption(o => o.setName("layout").setDescription("Visual layout style").setRequired(true)
                .addChoices(
                    { name: "🧊 Minimal - clean, no clutter", value: "minimal" },
                    { name: "🎪 Banner - bold title, big divider", value: "banner" },
                    { name: "💻 Terminal - code-block hacker style", value: "terminal" },
                    { name: "💀 Vault - dark black-market style", value: "vault" },
                    { name: "🖼️ Gallery - features as a grid", value: "gallery" }
                ))
            .addStringOption(o => o.setName("color").setDescription("Embed color theme").setRequired(true)
                .addChoices(
                    { name: "💜 Purple", value: "#9D00FF" },
                    { name: "🔴 Crimson", value: "#FF0033" },
                    { name: "🟡 Gold", value: "#FFD700" },
                    { name: "🟢 Emerald", value: "#00D632" },
        ...
        async execute(interaction){
            ...
            const modal = new ModalBuilder()
                .setCustomId(`blueprint_form_${layout}_${color.replace("#", "")}`)
                .setTitle("Design Your Panel");

            const titleInput = new TextInputBuilder()
                .setCustomId("bp_title")
                .setLabel("Panel Title")
                .setPlaceholder("e.g. Damascus Camo")
                .setStyle(TextInputStyle.Short)
                .setMaxLength(100)
    ...
    // -- /heist: live community event - crew up, timer runs out, loot drops --
    {
        data: new SlashCommandBuilder()
            .setName("heist")
            .setDescription("🏴‍☠️ Start a community heist event")
            .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
            .addIntegerOption(o => o.setName("seconds").setDescription("How long the crew has to join (default 60)").setRequired(false))
            .addIntegerOption(o => o.setName("max_winners").setDescription("Max number of winners (default 3)").setRequired(false)),

        async execute(interaction){

            const seconds = interaction.options.getInteger("seconds") || 60;
            const maxWinners = interaction.options.getInteger("max_winners") || 3;

            const embed = buildHeistEmbed({ participantCount: 0, secondsLeft: seconds, ended: false });
            const row = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId("heist_join").setLabel("Join the Crew").setEmoji("🏴‍☠️").setStyle(ButtonStyle.Danger)
            );

            const message = await interaction.reply({ embeds: [embed], components: [row], fetchReply: true });

            const heists = heistDB.read();
            heists[message.id] = {
            ...
            heistDB.write(heists);
        }
    },

    // -- /kingpin: crown (or check) the current top customer ---------------------
    {
        data: new SlashCommandBuilder()
            .setName("kingpin")
            .setDescription("👑 View or crown the current Kingpin")
            .addUserOption(o => o.setName("crown").setDescription("Staff only: crown this user as Kingpin").setRequired(false))
            .addStringOption(o => o.setName("reason").setDescription("Why they're being crowned").setRequired(false)),

        async execute(interaction){

            const target = interaction.options.getUser("crown");

            if(target){
                if(!interaction.member.permissions.has(PermissionFlagsBits.ManageGuild)){
                    return interaction.reply({ content: "❌ Only staff can crown a new Kingpin.", ephemeral: true });
                }

                const reason = interaction.options.getString("reason") || "Outstanding loyalty and support";
                const record = { userId: target.id, crownedAt: Date.now(), crownedBy: interaction.user.id, reason };
                kingpinDB.write(record);

                if(CONFIG.KINGPIN_ROLE_ID){
                    const oldKingpin = interaction.guild.members.cache.find(m => m.roles.cache.has(CONFIG.KINGPIN_ROLE_ID));
                    if(oldKingpin && oldKingpin.id !== target.id){
                        await oldKingpin.roles.remove(CONFIG.KINGPIN_ROLE_ID).catch(() => {});
                    }
                    const member = await interaction.guild.members.fetch(target.id).catch(() => null);
                    if(member) await member.roles.add(CONFIG.KINGPIN_ROLE_ID).catch(() => {});
                }

                const embed = new EmbedBuilder()
                    .setColor("#FFD700")
                    .setTitle("👑 A NEW KINGPIN RISES 👑")
                    .setDescription(
                        "```\n💎 THE THRONE HAS CHANGED HANDS 💎\n```\n" +
                        `${target} has been crowned **Kingpin** of ${CONFIG.BRAND_NAME}.\n\n` +
                        `**Reason:** ${reason}\n\n` +
                        "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"
                    )
                    .setFooter({ text: `Crowned by ${interaction.user.tag}` })
                    .setTimestamp();

                return interaction.reply({ embeds: [embed] });
            }

            const current = kingpinDB.read();
            if(!current){
                return interaction.reply({ content: "👑 No Kingpin has been crowned yet. Staff can use `/kingpin crown:@user` to crown one.", ephemeral: true });
            }

            const daysHeld = Math.floor((Date.now() - current.crownedAt) / (24 * 60 * 60 * 1000));
            const embed = new EmbedBuilder()
                .setColor("#FFD700")
                .setTitle("👑 THE CURRENT KINGPIN 👑")
                .setDescription(
                    `<@${current.userId}> has held the throne for **${daysHeld} day${daysHeld === 1 ? "" : "s"}**.\n\n` +
                    `**Reason:** ${current.reason}\n**Crowned by:** <@${current.crownedBy}>`
                )
                .setFooter({ text: CONFIG.BRAND_NAME })
                .setTimestamp();
    ...
    // -- /blacksite: dramatic multi-stage animated flash-deal reveal -----------
    {
        data: new SlashCommandBuilder()
            .setName("blacksite")
            .setDescription("🛰️ Reveal a hidden flash deal, with a dramatic build-up")
            .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
            .addStringOption(o => o.setName("service").setDescription("What's the flash deal for?").setRequired(true))
            .addIntegerOption(o => o.setName("discount_percent").setDescription("Discount percent").setRequired(true))
            .addIntegerOption(o => o.setName("duration_minutes").setDescription("How long the deal stays live (default 15)").setRequired(false)),

        async execute(interaction){

            const service = interaction.options.getString("service");
            const percent = interaction.options.getInteger("discount_percent");
            const minutes = interaction.options.getInteger("duration_minutes") || 15;

            const stageColor = "#1a1a1a";

            await interaction.reply({
                embeds: [new EmbedBuilder().setColor(stageColor).setTitle("🛰️ Establishing secure connection...").setDescription("```\n[■□□□□□□□□□] 10%\n```")]
            });
            await new Promise(r => setTimeout(r, 1200));

            await interaction.editReply({
                embeds: [new EmbedBuilder().setColor(stageColor).setTitle("📡 Triangulating black site coordinates...").setDescription("```\n[■■■■□□□□□□] 40%\n```")]
            });
            await new Promise(r => setTimeout(r, 1200));

            await interaction.editReply({
                embeds: [new EmbedBuilder().setColor(stageColor).setTitle("🔓 Decrypting vault access...").setDescription("```\n[■■■■■■■□□□] 75%\n```")]
            });
            await new Promise(r => setTimeout(r, 1200));
            ...
            const code = `SITE-${Math.floor(1000 + Math.random() * 9000)}`;
            const discounts = discountsDB.read();
            discounts[code] = { percent, usesLeft: 999, createdBy: "blacksite", expiresAt: Date.now() + minutes * 60 * 1000 };
            discountsDB.write(discounts);

            const finalEmbed = new EmbedBuilder()
                .setColor("#FF0033")
                .setTitle("🚨 BLACK SITE LOCATED 🚨")
                .setDescription(
                    "```\n🖤 CLASSIFIED DEAL UNLOCKED 🖤\n```\n" +
                    `## ${service}\n\n` +
                    `**${percent}% OFF** — code \`${code}\`\n\n` +
                    `⏱️ This site self-destructs in **${minutes} minutes**. Move fast.\n\n` +
                    "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"
                )
                .setFooter({ text: `${CONFIG.BRAND_NAME} • Use /redeem before it's gone` })
                .setTimestamp();

            const claimRow = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId("blacksite_claim").setLabel("Claim This Deal").setEmoji("🎯").setStyle(ButtonStyle.Danger)
            );

            await interaction.editReply({ embeds: [finalEmbed], components: [claimRow] });

            setTimeout(async () => {
                const d = discountsDB.read();
    ...
    // -- Staff: toggle "text only" mode on a channel (blocks all images and
    // attachments there - great for stopping scam screenshot spam) ----------
    {
        data: new SlashCommandBuilder()
            .setName("textonly")
            .setDescription("Staff: block images/attachments in a channel")
            .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
            .addStringOption(o => o.setName("action").setDescription("Enable or disable").setRequired(true)
                .addChoices(
                    { name: "✅ Enable (block images here)", value: "enable" },
                    { name: "❌ Disable (allow images again)", value: "disable" },
                    { name: "📋 List all text-only channels", value: "list" }
                ))
            .addChannelOption(o => o.setName("channel").setDescription("Which channel (defaults to this one)").setRequired(false)),

        async execute(interaction){

            const action = interaction.options.getString("action");
            const channel = interaction.options.getChannel("channel") || interaction.channel;
            const list = textOnlyChannelsDB.read();

            if(action === "list"){
                const allIds = [...new Set([...CONFIG.TEXT_ONLY_CHANNEL_IDS, ...list])];
                const desc = allIds.length ? allIds.map(id => `<#${id}>`).join("\n") : "No text-only channels set.";
                return interaction.reply({ content: `📋 **Text-only channels:**\n${desc}`, ephemeral: true });
            }

            if(action === "enable"){
                if(!list.includes(channel.id)){
                    list.push(channel.id);
                    textOnlyChannelsDB.write(list);
                }
                return interaction.reply({ content: `✅ ${channel} is now text-only - images and attachments will be auto-deleted there.` });
            }

            // disable
            const updated = list.filter(id => id !== channel.id);
            textOnlyChannelsDB.write(updated);

            if(CONFIG.TEXT_ONLY_CHANNEL_IDS.includes(channel.id)){
                return interaction.reply({ content: `⚠️ ${channel} is still text-only because it's hardcoded in the \`TEXT_ONLY_CHANNEL_IDS\` env var - remove it there too to fully disable.` });
            }

            await interaction.reply({ content: `✅ ${channel} is no longer text-only - images are allowed again.` });
        }
    }
...
const buttonHandlers = {
    ...
    async leave_vouch(interaction){
        ...
        const stepEmbed = new EmbedBuilder()
            .setColor("#FFD700")
            .setTitle("⭐ Step 1 of 4 — Rate Your Experience")
    ...
    async vouch_photo_yes(interaction){
        ...
        await interaction.update({
            embeds: [
                new EmbedBuilder()
                    .setColor("#FFD700")
                    .setTitle("📸 Step 4 of 4 — Upload Your Screenshot")
    ...
    // -- Wraith: reroll ghost codename ------------------------------------------
    async wraith_reroll(interaction){
        const users = usersDB.read();
        const record = users[interaction.user.id] || { vouches: 0 };
        const tag = WRAITH_NAMES[Math.floor(Math.random() * WRAITH_NAMES.length)];
        record.ghostTag = tag;
        ...
        const embed = new EmbedBuilder()
            .setColor("#2C2F33")
            .setTitle("🔁 New Ghost Identity")
            .setDescription(`You are now known as **Ghost: ${tag}**.`)
            .setFooter({ text: CONFIG.BRAND_NAME })
            .setTimestamp();
    ...
    // -- Sanctum vault access button - opens a real private ticket -------------
    async sanctum_access(interaction){
        await createTicketFor(interaction, "sanctum", "Sanctum Vault Access");
    },

    // -- Heist: join the crew on an active heist event --------------------------
    async heist_join(interaction){

        const heists = heistDB.read();
        const heist = heists[interaction.message.id];

        if(!heist || heist.ended){
            return interaction.reply({ content: "❌ This heist has already ended.", ephemeral: true });
        }
        if(Date.now() >= heist.endTime){
            return interaction.reply({ content: "❌ Too slow - the timer already ran out!", ephemeral: true });
        }
        if(heist.participants.includes(interaction.user.id)){
            return interaction.reply({ content: "🏴‍☠️ You're already in the crew!", ephemeral: true });
        }

        heist.participants.push(interaction.user.id);
        heistDB.write(heists);

        const secondsLeft = Math.max(0, Math.ceil((heist.endTime - Date.now()) / 1000));
        const embed = buildHeistEmbed({ participantCount: heist.participants.length, secondsLeft, ended: false });

        await interaction.update({ embeds: [embed] }).catch(() =>
            interaction.reply({ content: `✅ You joined the crew! (${heist.participants.length} total)`, ephemeral: true })
        );
    },

    // -- Black Site: claim the flash deal by opening a ticket ------------------
    async blacksite_claim(interaction){

        const description = interaction.message?.embeds?.[0]?.description || "";
        const serviceMatch = description.match(/## (.+)/);
        const codeMatch = description.match(/`([A-Z0-9-]+)`/);

        const service = serviceMatch ? serviceMatch[1].trim() : "Black Site Deal";
        const code = codeMatch ? codeMatch[1] : null;

        await createTicketFor(interaction, "blacksite", `Black Site: ${service}${code ? ` (code ${code})` : ""}`);
    },
...
async function handleVouchRating(interaction, rating){
    ...
    await interaction.update({
        embeds: [
            new EmbedBuilder()
                .setColor("#FFD700")
                .setTitle("💬 Step 2 of 4 — Tell Us About It")
    ...
    const anonRow = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId("vouch_anon_yes").setLabel("Post Anonymously").setEmoji("🕶️").setStyle(ButtonStyle.Secondary),
        new ButtonBuilder().setCustomId("vouch_anon_no").setLabel("Show My Name").setEmoji("👤").setStyle(ButtonStyle.Primary)
    );
    ...
    await interaction.channel.send({
        embeds: [
            new EmbedBuilder()
                .setColor("#FFD700")
                .setTitle("🕶️ Step 3 of 4 — Post Publicly or Anonymously?")
                .setDescription("Totally optional — want your name shown on the vouch, or posted as an anonymous customer?")
                .setFooter({ text: CONFIG.BRAND_NAME })
        ],
        components: [anonRow]
    }).catch(() => {});
...
async function promptForVouchPhoto(channel){
    ...
    await channel.send({
        embeds: [
            new EmbedBuilder()
                .setColor("#FFD700")
                .setTitle("📸 Last Step — Add a Screenshot?")
...
async function finalizeVouch(interaction, session, imageUrl){
    ...
    const embed = buildVouchResultEmbed({
        ...
        ghostTag: users[session.userId].ghostTag || null
    });
...
const modalHandlers = {
    ...
    // -- Blueprint: builds one of 5 layout styles based on what was chosen
    // in the slash command options (encoded in the modal's customId since
    // modals can't carry extra state any other way). Reuses buy_custom_<slug>
    // so it opens tickets exactly like /custompanel does. -------------------
    async blueprint_form(interaction){

        const parts = interaction.customId.split("_"); // ["blueprint","form",layout,colorHex]
        const layout = parts[2];
        const color = `#${parts[3]}`;

        const title = interaction.fields.getTextInputValue("bp_title");
        const slug = interaction.fields.getTextInputValue("bp_slug").trim().toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9_-]/g, "").replace(/-+/g, "-").replace(/^-|-$/g, "");
        const price = interaction.fields.getTextInputValue("bp_price");
        const features = interaction.fields.getTextInputValue("bp_features").split("\n").map(f => f.trim()).filter(Boolean);
        const buttonLabel = interaction.fields.getTextInputValue("bp_button_label") || "Open Ticket";

        if(!slug){
            return interaction.reply({ content: "❌ That slug isn't valid - use letters, numbers, dashes or underscores only. Run `/blueprint` again.", ephemeral: true });
        }

        await interaction.reply({
            content: "📸 Last step! Upload an image for the panel right here (as an attachment), or type `skip` to leave it blank. You have 3 minutes.",
            ephemeral: true
        });
        ...
        try{
            ...
            return interaction.followUp({ content: "⌛ Timed out waiting for an image. Run `/blueprint` again when you're ready.", ephemeral: true });
        }
        ...
        if(!attachment && !skipped){
            return interaction.followUp({ content: "❌ That wasn't an image or `skip`. Run `/blueprint` again.", ephemeral: true });
        }
        ...
        const embed = new EmbedBuilder().setColor(color).setFooter({ text: CONFIG.BRAND_NAME }).setTimestamp();

        if(layout === "minimal"){
            embed.setTitle(title).setDescription(`${features.join(" • ")}\n\n**${price}**`);
            ...
            embed.setTitle(`> ${title}`)
                .setDescription("```\n" + features.map(f => `[+] ${f}`).join("\n") + `\n[$] PRICE: ${price}\n\`\`\``);
        }else if(layout === "vault"){
            embed.setTitle(`💀 ${title.toUpperCase()} 💀`)
                .setDescription(`🔓 UNDERGROUND ACCESS\n\n${features.map(f => `⚡ ${f}`).join("\n")}\n\n💰 \`${price}\`\n\n🔒 Discreet & staff-handled`);
        }else if(layout === "gallery"){
            embed.setTitle(title).setDescription(`**${price}**`)
                .addFields(features.map((f, i) => ({ name: `Feature ${i + 1}`, value: f, inline: true })));
        }
...
async function handleSecurityChecks(message){
    ...
    // -- Known scam domains (e.g. fake "MrBeast crypto giveaway" screenshots
    // pointing to winleg.com and similar) - blocked everywhere, staff exempt --
    const lowerContent = message.content.toLowerCase();
    const matchedDomain = CONFIG.BLOCKED_DOMAINS.find(d => d && lowerContent.includes(d.toLowerCase()));
    if(!isStaff && matchedDomain){
        await message.delete().catch(() => {});
        const warning = await message.channel.send({
            content: `${message.author}, that link is a known scam and has been removed. Never enter your wallet/payment info on sites like that.`
        }).catch(() => null);
        if(warning) setTimeout(() => warning.delete().catch(() => {}), 8000);

        await securityLog(message.guild, {
            title: "🚨 Scam Link Blocked",
            description: `**User:** ${message.author} (${message.author.id})\n**Channel:** ${message.channel}\n**Matched domain:** \`${matchedDomain}\`\n**Content:** ${message.content.slice(0, 300)}`,
            color: "#FF0000"
        });
    ...
    // -- Text-only channels: no images/attachments allowed at all (kills
    // scam screenshots along with everything else) - staff exempt --
    const liveTextOnlyChannels = textOnlyChannelsDB.read();
    const isTextOnlyChannel = CONFIG.TEXT_ONLY_CHANNEL_IDS.includes(message.channel.id) || liveTextOnlyChannels.includes(message.channel.id);
    if(!isStaff && isTextOnlyChannel){
        const hasAttachment = message.attachments.size > 0;
        const hasImageEmbed = message.embeds.some(e => e.image || e.thumbnail);
        if(hasAttachment || hasImageEmbed){
            await message.delete().catch(() => {});
            const warning = await message.channel.send({
                content: `${message.author}, this channel is text-only - images and attachments aren't allowed here.`
            }).catch(() => null);
            if(warning) setTimeout(() => warning.delete().catch(() => {}), 6000);

            await securityLog(message.guild, {
                title: "🖼️ Image Blocked (Text-Only Channel)",
                description: `**User:** ${message.author} (${message.author.id})\n**Channel:** ${message.channel}`
            });
...
// time has arrived. --------------------------------------------------------
// -- Called on a fast (10s) timer from bot.js: ends any heist whose timer
// has run out, picks winners, hands out loot, and edits the message. -------
async function checkHeists(client){

    const heists = heistDB.read();
    let changed = false;

    for(const [messageId, heist] of Object.entries(heists)){

        if(heist.ended || Date.now() < heist.endTime) continue;
        ...
        try{
            ...
            const winnerCount = Math.min(heist.maxWinners, heist.participants.length);
            const shuffled = [...heist.participants].sort(() => Math.random() - 0.5);
            const chosenIds = shuffled.slice(0, winnerCount);

            const rarityTable = [
                { threshold: 5, percent: 30, emoji: "🌟" },
                { threshold: 20, percent: 20, emoji: "💜" },
                { threshold: 50, percent: 10, emoji: "🔷" },
                { threshold: 100, percent: 5, emoji: "⚪" }
            ];

            const discounts = discountsDB.read();
            const winners = chosenIds.map(userId => {
                const roll = Math.random() * 100;
                const tier = rarityTable.find(t => roll < t.threshold) || rarityTable[rarityTable.length - 1];
                const code = `HEIST-${userId.slice(-4)}${Math.floor(100 + Math.random() * 900)}`;
                discounts[code] = { percent: tier.percent, usesLeft: 1, createdBy: "heist" };
                return { userId, percent: tier.percent, emoji: tier.emoji, code };
            });
            discountsDB.write(discounts);

            const embed = buildHeistEmbed({ participantCount: heist.participants.length, ended: true, winners });
            await message.edit({ embeds: [embed], components: [] }).catch(() => {});

            if(winners.length){
                await channel.send({
                    content: `🎉 ${winners.map(w => `<@${w.userId}>`).join(", ")} — check your loot above! Use \`/redeem\` to claim it.`
                }).catch(() => {});
            }

        }catch(err){
            console.log(`[heist] ❌ could not resolve heist ${messageId}: ${err.message}`);
        }
    }

    if(changed) heistDB.write(heists);
}
