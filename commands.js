...
// 14 FIRE NEW COMMANDS - BLACK MARKET VIBES
const tiers = { data: new SlashCommandBuilder().setName("tiers").setDescription("💎 view our VIP tier system"), execute: async (i) => { const e = new EmbedBuilder().setColor("#9D00FF").setTitle("💎 TIER SYSTEM 💎").addFields({ name: "🥉 BRONZE", value: "```Entry level\n1-5 purchases\nStandard support```", inline: true }, { name: "🥈 SILVER", value: "```Growing trust\n6-15 purchases\n5% off all```", inline: true }, { name: "🥇 GOLD", value: "```Regular\n16-30 purchases\n10% off all```", inline: true }, { name: "💜 PLATINUM", value: "```Elite\n31+ purchases\n15% off all```", inline: true }, { name: "👑 APEX", value: "```Legendary\n100+ purchases\n20% off all```", inline: true }, { name: "🔓 BLACK MARKET", value: "```Exclusive\nPrivate access\nNo limits```", inline: true }).setFooter({ text: "RANK UP BY PURCHASING" }).setTimestamp(); await i.reply({ embeds: [e] }); } };
slashCommands.push(tiers);

const pricing = { data: new SlashCommandBuilder().setName("pricing").setDescription("💰 view service pricing"), execute: async (i) => { const e = new EmbedBuilder().setColor("#FF00FF").setTitle("💰 PRICING 💰").addFields({ name: "🚀 WZ BOOST", value: "```0-10: $15\n10-25: $25\n25-50: $40\n50+: $65```", inline: true }, { name: "🎮 MP BOOST", value: "```0-5: $10\n5-10: $18\n10-20: $30\n20+: $50```", inline: true }, { name: "🎨 CAMOS", value: "```Single: $5\n5 wpn: $20\nAll: $50\nGold: $75```", inline: true }).setFooter({ text: "TIER DISCOUNTS APPLY" }).setTimestamp(); await i.reply({ embeds: [e] }); } };
slashCommands.push(pricing);

const shop = { data: new SlashCommandBuilder().setName("shop").setDescription("🛒 browse our shop"), execute: async (i) => { const r = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId("shop_cat").setPlaceholder("Select...").addOptions({ label: "🚀 Boosts", value: "boosts" }, { label: "🎨 Cosmetics", value: "cosmetics" }, { label: "📱 Numbers", value: "numbers" }, { label: "💎 VIP", value: "vip" }, { label: "🎮 Accounts", value: "accounts" })); const e = new EmbedBuilder().setColor("#FF00FF").setTitle("🛒 BLACK MARKET SHOP 🛒").setDescription("Select a category").setFooter({ text: "CLICK TO BROWSE" }).setTimestamp(); await i.reply({ embeds: [e], components: [r], ephemeral: true }); } };
slashCommands.push(shop);

const stats = { data: new SlashCommandBuilder().setName("stats").setDescription("📊 server stats"), execute: async (i) => { const e = new EmbedBuilder().setColor("#9D00FF").setTitle("📊 STATS 📊").addFields({ name: "👥 MEMBERS", value: "```2,847 total\n1,234 active\n456 VIP```", inline: true }, { name: "💵 REVENUE", value: "```$34,580 (30d)\n1,847 orders\n98.7% satisfaction```", inline: true }, { name: "⚡ UPTIME", value: "```99.8% (30d)\n24/7 support\nZero downtime```", inline: true }).setFooter({ text: "REAL-TIME DATA" }).setTimestamp(); await i.reply({ embeds: [e] }); } };
slashCommands.push(stats);

const status = { data: new SlashCommandBuilder().setName("status").setDescription("⚡ service status"), execute: async (i) => { const e = new EmbedBuilder().setColor("#9D00FF").setTitle("⚡ SERVICE STATUS ⚡").addFields({ name: "🟢 WZ Ranked", value: "ONLINE", inline: true }, { name: "🟢 MP Ranked", value: "ONLINE", inline: true }, { name: "🟢 Camos", value: "ONLINE", inline: true }, { name: "🟢 Operators", value: "ONLINE", inline: true }, { name: "🟢 Numbers", value: "ONLINE", inline: true }, { name: "🟢 Middleman", value: "ONLINE", inline: true }).addField("📊 UPTIME", "99.8%").setFooter({ text: "ALL SYSTEMS GO" }).setTimestamp(); await i.reply({ embeds: [e] }); } };
slashCommands.push(status);

const referral = { data: new SlashCommandBuilder().setName("referral").setDescription("🔗 earn with referrals"), execute: async (i) => { const e = new EmbedBuilder().setColor("#FF00FF").setTitle("🔗 REFERRAL PROGRAM 🔗").setDescription("Earn 10% from every referral!").addFields({ name: "💰 HOW", value: "```Share link → They buy → You get 10%```", inline: false }).setFooter({ text: "GENERATE LINK BELOW" }).setTimestamp(); const r = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("gen_ref").setLabel("🔗 Get Link").setStyle(ButtonStyle.Success)); await i.reply({ embeds: [e], components: [r], ephemeral: true }); } };
slashCommands.push(referral);

const track = { data: new SlashCommandBuilder().setName("track").setDescription("📦 track orders"), execute: async (i) => { const e = new EmbedBuilder().setColor("#9D00FF").setTitle("📦 ORDER TRACKER 📦").addFields({ name: "🔵 PENDING", value: "```Order #WKD-001\n$40 | ETA: 4h```", inline: true }, { name: "🟢 IN PROGRESS", value: "```Order #WKD-002\n60% done```", inline: true }, { name: "✅ COMPLETED", value: "```Order #WKD-003\n4 days ago```", inline: true }).setFooter({ text: "OPEN TICKET FOR SUPPORT" }).setTimestamp(); await i.reply({ embeds: [e], ephemeral: true }); } };
slashCommands.push(track);

const perks = { data: new SlashCommandBuilder().setName("perks").setDescription("🎁 view member perks"), execute: async (i) => { const e = new EmbedBuilder().setColor("#9D00FF").setTitle("🎁 EXCLUSIVE PERKS 🎁").addFields({ name: "💜 VIP", value: "```5% off\nPriority support\nMoney-back guarantee```", inline: true }, { name: "👑 APEX", value: "```15% off\nDedicated handler\nCustom packages```", inline: true }, { name: "🔓 MARKET", value: "```20% off\nNo limits\nPrivate sellers```", inline: true }).setFooter({ text: "TIER UP BY PURCHASING" }).setTimestamp(); await i.reply({ embeds: [e] }); } };
slashCommands.push(perks);

const promo = { data: new SlashCommandBuilder().setName("promo").setDescription("🎟️ promo codes"), execute: async (i) => { const e = new EmbedBuilder().setColor("#FF00FF").setTitle("🎟️ ACTIVE PROMO CODES 🎟️").addFields({ name: "💜 PURPLE20", value: "```20% off boosts\nCode: PURPLE20```", inline: true }, { name: "💎 VIP2024", value: "```25% off all\nCode: VIP2024```", inline: true }, { name: "⚡ WELCOME10", value: "```10% off first\nCode: WELCOME10```", inline: true }).setFooter({ text: "UPDATED WEEKLY" }).setTimestamp(); await i.reply({ embeds: [e] }); } };
slashCommands.push(promo);

const testimonials = { data: new SlashCommandBuilder().setName("testimonials").setDescription("⭐ customer reviews"), execute: async (i) => { const e = new EmbedBuilder().setColor("#FF00FF").setTitle("⭐ CUSTOMER REVIEWS ⭐").addFields({ name: "@Shadow", value: "\"Best service. 50 wins in 2 days!\" ⭐⭐⭐⭐⭐", inline: false }, { name: "@Ghost", value: "\"Fast, reliable, professional.\" ⭐⭐⭐⭐⭐", inline: false }, { name: "@Apex", value: "\"Support is incredible.\" ⭐⭐⭐⭐⭐", inline: false }, { name: "@Titan", value: "\"25 vouches from me.\" ⭐⭐⭐⭐⭐", inline: false }).setFooter({ text: "500+ 5-STAR REVIEWS • 98.7% SATISFACTION" }).setTimestamp(); await i.reply({ embeds: [e] }); } };
slashCommands.push(testimonials);

const faq = { data: new SlashCommandBuilder().setName("faq").setDescription("❓ frequently asked questions"), execute: async (i) => { const e = new EmbedBuilder().setColor("#9D00FF").setTitle("❓ FAQ ❓").addFields({ name: "Q: Is my account safe?", value: "✅ Yes. Industry-standard security.", inline: false }, { name: "Q: How fast is delivery?", value: "✅ Instant for most. 24hr guaranteed.", inline: false }, { name: "Q: Can I get a refund?", value: "✅ 100% money-back within 48h.", inline: false }, { name: "Q: Do you get detected?", value: "✅ Zero bans. 2,800+ customers.", inline: false }).setFooter({ text: "MORE QUESTIONS? OPEN A TICKET" }).setTimestamp(); await i.reply({ embeds: [e] }); } };
slashCommands.push(faq);

const security = { data: new SlashCommandBuilder().setName("security").setDescription("🔒 security & privacy"), execute: async (i) => { const e = new EmbedBuilder().setColor("#9D00FF").setTitle("🔒 SECURITY & PRIVACY 🔒").addFields({ name: "🛡️ PROTECTION", value: "```Encryption\nAccount isolation\nVPN routing\n2FA support```", inline: true }, { name: "📊 DATA", value: "```Never shared\nAuto-deleted 30d\nGDPR compliant\nNo logs kept```", inline: true }).setFooter({ text: "INDUSTRY LEADING SECURITY" }).setTimestamp(); await i.reply({ embeds: [e] }); } };
slashCommands.push(security);

const contact = { data: new SlashCommandBuilder().setName("contact").setDescription("📞 contact information"), execute: async (i) => { const e = new EmbedBuilder().setColor("#FF00FF").setTitle("📞 CONTACT INFORMATION 📞").addFields({ name: "💬 DISCORD", value: "Open a ticket in server", inline: true }, { name: "🌐 WEBSITE", value: "https://weekendthrillers-services.base44.app", inline: true }, { name: "💰 PAYMENT", value: "$EvanJoyner8 (CashApp)", inline: true }, { name: "⏱️ RESPONSE TIME", value: "Under 5 minutes guaranteed", inline: false }).setFooter({ text: "ALWAYS AVAILABLE • NO QUESTIONS ASKED" }).setTimestamp(); await i.reply({ embeds: [e] }); } };
slashCommands.push(contact);

const rewards = { data: new SlashCommandBuilder().setName("rewards").setDescription("🏅 loyalty rewards program"), execute: async (i) => { const e = new EmbedBuilder().setColor("#9D00FF").setTitle("🏅 LOYALTY REWARDS 🏅").addFields({ name: "💳 EARN", value: "```$1 = 1 point\nReferral = 10 pts\nReview = 5 pts```", inline: true }, { name: "🎁 REDEEM", value: "```50 pts = $5\n100 pts = $15\n500 pts = $120```", inline: true }).setFooter({ text: "EARN WHILE YOU SPEND" }).setTimestamp(); await i.reply({ embeds: [e] }); } };
slashCommands.push(rewards);
