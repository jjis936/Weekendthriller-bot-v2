...
// ---------------------------------------------------------------------------
// BLACK MARKET EMBED UTILITIES
// ---------------------------------------------------------------------------

const BLACK_MARKET = {
    createPremiumEmbed: (title, description) => {
        return new EmbedBuilder()
            .setColor("#9D00FF")
            .setTitle(`🔓 ${title}`)
            .setDescription(description)
            .setFooter({ text: "⚡ WEEKENDTHRILLER BLACK MARKET ⚡", iconURL: CONFIG.BRAND_ICON_URL })
            .setTimestamp();
    },
    
    createExclusiveEmbed: (title) => {
        return new EmbedBuilder()
            .setColor("#FF00FF")
            .setTitle(`💀 ${title} 💀`)
            .setFooter({ text: "EXCLUSIVE | VIP ACCESS", iconURL: CONFIG.BRAND_ICON_URL })
            .setTimestamp();
    },
    
    createRankBoostEmbed: (service, details) => {
        const embed = new EmbedBuilder()
            .setColor("#9D00FF")
            .setTitle(`🚀 ${service.toUpperCase()} RANK BOOST 🚀`)
            .setDescription("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
            .addFields(
                { name: "⚡ SERVICE TIER", value: details.tier || "PREMIUM", inline: true },
                { name: "💰 PRICING", value: details.price || "CONTACT STAFF", inline: true },
                { name: "⏱️ ETA", value: details.eta || "VARIABLE", inline: true }
            )
            .setFooter({ text: "BLACK MARKET SERVICES • WEEKENDTHRILLER", iconURL: CONFIG.BRAND_ICON_URL })
            .setTimestamp();
        return embed;
    },

    createTicketEmbed: (service, status) => {
        return new EmbedBuilder()
            .setColor("#FF00FF")
            .setTitle(`🎫 ${service.toUpperCase()} TICKET`)
            .addField("📊 STATUS", `\`${status}\``, false)
            .addField("⚠️ DISCLAIMER", "Do not close this ticket unless service is complete. Refunds are non-refundable.", false)
            .setFooter({ text: "WEEKENDTHRILLER SERVICES", iconURL: CONFIG.BRAND_ICON_URL })
            .setTimestamp();
    }
};
...
// BLACK MARKET SHOWCASE COMMAND
const blackMarketShowcase = {
    data: new SlashCommandBuilder()
        .setName("blackmarket")
        .setDescription("🔓 view our exclusive black market services"),
        ...
        embeds.push(
            new EmbedBuilder()
                ...
                .setDescription(
                    ...
                    "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n" +
                    "Exclusive. Premium. No Compromises.\n" +
                    "```\n\n" +
                    "🔓 **VIP TIER SERVICES**\n" +
                    "└─ Rank Boosts | Camos | Operators | Nukes\n\n" +
                    "⚡ **INSTANT DELIVERY**\n" +
                    "└─ Expert handlers | Verified vouches | Secure trades\n\n" +
                    "🎯 **100% DISCRETION**\n" +
                    "└─ Private tickets | No history | Zero logs"
                )
                .setFooter({ text: "AUTHENTICATION REQUIRED", iconURL: CONFIG.BRAND_ICON_URL })
                .setTimestamp()
        );
        
        // embed 2: services grid
        embeds.push(
            new EmbedBuilder()
                .setColor("#FF00FF")
                .setTitle("🛍️ SERVICE CATALOG")
                .addFields(
                    { name: "🚀 WZ RANKED BOOST", value: "```Elite multiplayer carry\nAccount security: MAXIMUM```", inline: true },
                    { name: "🎮 MP RANKED BOOST", value: "```Competitive edge included\nFull transparency on methods```", inline: true },
                    { name: "🎨 CAMO UNLOCKS", value: "```All weapons, instant access\nZero detection risk```", inline: true },
                    { name: "👾 OPERATORS", value: "```Endgame exclusive skins\nVIP-only inventory```", inline: true },
                    { name: "💣 NUKE SERVICES", value: "```Custom loadouts\nStrike package assistance```", inline: true },
                    { name: "📱 NUMBER RENTAL", value: "```SMS verification ready\nGlobal coverage available```", inline: true }
                )
                .setFooter({ text: "ALL SERVICES BACKED BY OUR GUARANTEE", iconURL: CONFIG.BRAND_ICON_URL })
                .setTimestamp()
        );
        
        // embed 3: pricing & benefits
        embeds.push(
            new EmbedBuilder()
                .setColor("#9D00FF")
                .setTitle("💎 PREMIUM BENEFITS")
                .setDescription(
                    "✅ **SPEED** - Next-day delivery on most services\n" +
                    "✅ **SECURITY** - Account protection guaranteed\n" +
                    "✅ **SUPPORT** - 24/7 dedicated staff\n" +
                    "✅ **VOUCH** - 500+ verified customer reviews\n" +
                    "✅ **REFUND** - Satisfaction guarantee or money back\n\n" +
                    "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n" +
                    "**Open a ticket to get started** `/ticket`"
                )
                .setFooter({ text: "WEEKENDTHRILLER SERVICES", iconURL: CONFIG.BRAND_ICON_URL })
                .setTimestamp()
        );
